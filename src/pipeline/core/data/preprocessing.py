...
def get_transforms(
    ...
    mean: list,
    std: list,
) -> dict[pipeline.schemas.constants.Phase, torchvision.transforms.Compose]:
    """
    ...
    data_transforms = {
        pipeline.schemas.constants.Phase.TRAINING: torchvision.transforms.Compose(
            spatial_augmentation + color_augmentation + resize_and_padding + normalization
        ),
        pipeline.schemas.constants.Phase.VALIDATION: torchvision.transforms.Compose(resize_and_padding + normalization),
    }
...
class Resize:
    ...
    def __call__(self, image: np.ndarray) -> np.ndarray:
        ...
        if self.padding is pipeline.schemas.constants.PaddingType.BOTTOMRIGHT:
            output_image[
                :image_height,
                :image_width,
            ] = image
        elif self.padding is pipeline.schemas.constants.PaddingType.BOTTOMLEFT:
            output_image[
                :image_height,
                self.input_width - image_width :,
            ] = image
        elif self.padding is pipeline.schemas.constants.PaddingType.TOPLEFT:
            output_image[
                self.input_height - image_height :,
                :image_width,
            ] = image
        elif self.padding is pipeline.schemas.constants.PaddingType.TOPRIGHT:
            output_image[
                self.input_height - image_height :,
                self.input_width - image_width :,
            ] = image
        elif self.padding is pipeline.schemas.constants.PaddingType.CENTER:
