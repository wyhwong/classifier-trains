...
import pipeline.core.data
import pipeline.core.model
import pipeline.core.utils
import pipeline.core.visualization
import pipeline.env
import pipeline.logger
import pipeline.schemas.constants


torch.manual_seed(pipeline.env.RANDOM_SEED)
np.random.seed(pipeline.env.RANDOM_SEED)
...
class ModelFacade:
    ...
    def __init__(self, setting: dict[str, Any]) -> None:
        ...
        self._data_transforms: dict[pipeline.schemas.constants.Phase, Any] = {}
        self._dataloaders: dict[pipeline.schemas.constants.Phase, torch.utils.data.DataLoader] = {}
    ...
    def _initialize(self) -> None:
        ...
        pipeline.core.utils.check_and_create_dir(pipeline.env.RESULT_DIR)
        num_experiment = len(glob(f"{pipeline.env.RESULT_DIR}/*")) + 1
        self._output_dir = f"{pipeline.env.RESULT_DIR}/{num_experiment}"
        if self._setting["experiment_label"]:
            self._output_dir += "_" + self._setting["experiment_label"]
        pipeline.core.utils.check_and_create_dir(self._output_dir)
        pipeline.core.utils.save_as_yml(f"{self._output_dir}/setting.yml", self._setting)
    ...
    def _initialize_model(self) -> None:
        ...
        self._model = pipeline.core.model.initialize.initialize_model(
            backbone=pipeline.schemas.constants.ModelBackbone(self._setting["model"]["backbone"]),
    ...
    def _initialize_data_transform(self) -> None:
        ...
        spatial_augmentation = pipeline.core.data.preprocessing.get_spatial_transforms(
        ...
        color_augmentation = pipeline.core.data.preprocessing.get_color_transforms(
        ...
        resize_and_padding = pipeline.core.data.preprocessing.get_resize_and_padding_transforms(
            width=self._setting["preprocessing"]["width"],
            height=self._setting["preprocessing"]["height"],
            maintain_aspect_ratio=self._setting["preprocessing"]["resize_and_padding"]["maintain_aspect_ratio"],
            interpolation=pipeline.schemas.constants.InterpolationType(
                self._setting["preprocessing"]["resize_and_padding"]["interpolation"]
            ),
            padding=pipeline.schemas.constants.PaddingType(
                self._setting["preprocessing"]["resize_and_padding"]["padding"]
            ),
        )
        self._data_transforms = pipeline.core.data.preprocessing.get_transforms(
    ...
    def _initialize_dataset(self) -> None:
        ...
        for phase in [pipeline.schemas.constants.Phase.TRAINING, pipeline.schemas.constants.Phase.VALIDATION]:
            ...
            pipeline.core.visualization.dataset.get_dataset_preview(
        ...
        pipeline.core.data.mapping.save_class_mapping(
            dataset=image_dataset,
    ...
    def _run_training(self) -> None:
        ...
        optimizer = pipeline.core.model.initialize.initialize_optimizer(
            params=self._model.parameters(),
            optimizier=pipeline.schemas.constants.OptimizerType(self._setting["training"]["optimizer"]["name"]),
        ...
        scheduler = pipeline.core.model.initialize.initialize_scheduler(
            scheduler=pipeline.schemas.constants.SchedulerType(self._setting["training"]["scheduler"]["name"]),
            optimizer=optimizer,
        ...
        self._model, self._best_weights, self._last_weights, loss, accuracy = pipeline.core.model.train.train_model(
            model=self._model,
            ...
            scheduler=scheduler,
            num_epochs=self._setting["training"]["num_epochs"],
            best_criteria=pipeline.schemas.constants.BestCriteria(self._setting["training"]["best_criteria"]),
        )
        self._loss = pd.DataFrame(loss)
        pipeline.core.visualization.performance.loss_curve(df=self._loss, output_dir=self._output_dir)
        self._accuracy = pd.DataFrame(accuracy)
        pipeline.core.visualization.performance.accuracy_curve(df=self._accuracy, output_dir=self._output_dir)
    ...
    def _run_evaluation(self) -> None:
        ...
        image_dataset = datasets.ImageFolder(
            ...
            self._data_transforms[pipeline.schemas.constants.Phase.VALIDATION],
        )
        ...
        mapping = pipeline.core.utils.load_yml(mapping_path)
        models = []
        model_names = []
        for model_for_eval in self._setting["evaluation"]["models"]:
            ...
            model = pipeline.core.model.initialize.initialize_model(
                backbone=pipeline.schemas.constants.ModelBackbone(model_for_eval["backbone"]),
                weights="DEFAULT",
                num_classes=len(mapping.keys()),
                unfreeze_all_params=False,
            )
            pipeline.core.model.load.load_model(model, weights_path)
            models.append(model)
            model_names.append(model_for_eval["name"])

            y_true, y_pred, _ = pipeline.core.model.inference.predict(model, dataloader)
            pipeline.core.visualization.performance.confusion_matrix(
                y_true=y_true,
                y_pred=y_pred,
                output_dir=self._output_dir,
        ...
        pipeline.core.visualization.performance.roc_curves(
            models=models,
    ...
    def _run_export(self) -> None:
        ...
        if self._setting["export"]["save_last_weight"]:
            ...
            pipeline.core.model.load.save_weights(self._last_weights, model_path)

        if self._setting["export"]["save_best_weight"]:
            model_path = f"{self._output_dir}/best_model.pt"
            pipeline.core.model.load.save_weights(self._best_weights, model_path)
